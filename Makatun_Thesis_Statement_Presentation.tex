...
\begin{itemize}
\item $Y_{j} \in \{0,n\}$ selected processing node. 
...
\begin{frame}\frametitle{Summary of constraints}
\vspace{-5mm}
\begin{footnotesize}
	\begin{block}{Planning stage}
	\vspace{-2mm}
\begin{columns}[c] % the "c" option specifies center vertical alignment
...
\end{block}
\begin{block}{Scheduling stage}
\vspace{-2mm}
\begin{itemize}
\item Outgoing transfer starts after the incoming one.
\item Jobs starts after the input file transfer is finished.
\item Output file is transferred after the job is finished.
\item Space reservation at destination node is made when transfer starts.
\item File can be deleted from start node of a link after the transfer.
\item Space for output is reserved when a job starts.
\item When a job finishes its input file can be deleted.
\end{itemize}
\end{block}
