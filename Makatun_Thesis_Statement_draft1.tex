...
\inst{Faculty of Informatics, Masaryk University, Czech Republic }
...
constraint programming, Grid, Cloud, computing, data processing, data transfer, data production, planning, scheduling, optimization, computational jobs, batch system.
\end{keywords}
...
replication of frequently used files to more sites while the jobs were executed
...
processing are initially placed. All the output files have to be transferred
...
For example, the transformation of the Grid from Figure \ref{sites} is illustrated at Figure \ref{real_network} (HPSS node was excluded from the consideration).
...
and $k_{0}$ is the total size of available input files at $c_{0}$. A general case of a resulting network is represented at Figure \ref{network_general}. We denote
...
within time interval $\Delta T$ (it is limited by the available disc space at the
...
Since $w_{i}$ was defined in Subsection \ref{inproblem} as maximal amount of input data that can be transferred to the computing node $c_{i}$, we can now define it using Equation \ref{newIn}:
\begin{equation}
w_{i} =
Disk_{i} - I_{i}^{in} - I_{i}^{out} + \frac{1 - \beta}{\alpha_{i}} \cdot NCPU_{i} \cdot \Delta T + Del_{i}^{out}
\label{w}
...
\item[Optimized:] This strategy is based on the model proposed in this Section.
\end{description}
...
Distributed data processing in HENP as well as other data intensive applications for Grid require further improvement of load planning and job scheduling. Inspired by success of global planning for distributed systems \cite{Zerola} and for media streams \cite{Rudova} we extended \cite{ACAT_cp,MISTA} these approaches to the entire data production in HENP, combining job scheduling and data management.  

In Section \ref{Network_flow} we proposed \cite{MISTA} a model of distributed data production, where all the files from a single source has to be processed once and transferred back. This model allows planning of WAN, storage and CPU loads using the network flow maximization approach. 

In Section \ref{CP_approach} a model for scheduling of data production over Grid was formulated in form of constraint satisfaction problem and solved using constraint programming \cite{ACAT_cp}. The simulations based on data extracted from log files of batch and data management systems of the STAR experiment has shown that the proposed global planning approach systematically provides a smaller makespan and adapts to the increase of transfer overheads better then the other simulated  heuristics.
