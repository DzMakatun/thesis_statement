...
{data production} 
1 job per file, Single source, large output
overall compleation time, job order is unimportant.

{user analysis} 
Many sources, several jobs per file, output size is negligible, inaccurate time estimations. 
User fairness is important

Sections \ref{Network_flow} and \ref{CP_approach} is focused on scheduling for data production, while Section \ref{Cache} compares caching strategies for a mixed workload. 
